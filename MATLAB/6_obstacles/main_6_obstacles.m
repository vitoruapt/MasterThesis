...
N = 6;          % Random number of obstacles (1,2,3,4,5,6)
road.length = 150;         % length of the street
car.minimum_dist=40;       % minimum distance between 2 cars
  
obstacle_random_X=obstacleRandomGenerationX(road.length,car.minimum_dist,N);
obstacle_cresc_X=sort(obstacle_random_X);
...
for i=1:N
    ...
    %obstacle(i).X = obstacle_cresc_X(i);         % Initial positions of the N obstacles   
    %obstacle(i).Y = randi([-(road.laneWidth*road.lanes)/4 (road.laneWidth*road.lanes)/4],1,1);   
