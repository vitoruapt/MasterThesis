...
\begin{document}
\begin{tikzpicture}[every node/.style={transform shape},scale=1.75]
%AXIS
    \draw [<->,thick] (-0.3,0.5) node (yaxis) [ right, yshift=-2pt] {$y$}
    |- ++(0.5,-0.5) node (xaxis) [above] {$x$};
% CAR
\draw[green,
  postaction={
    decorate,
    decoration={
      ...
      {
        \node[transform shape] {\includegraphics[width=1cm]{car}};
      }
    }
  }
]
...
\draw [|->,semithick,>=stealth] (-0.3,-0.2) -- (1,-0.2)  node[midway, yshift=+3pt,font=\tiny,below] {$x_{\min}$};

\draw [|->,semithick,>=stealth] (-0.3,-0.47) -- (2.65,-0.47)  node[pos=0.75, yshift=-3pt,font=\tiny,above] {$x_{\max}$};
...
postaction={
	...
	decoration={
		...
		{
			\node[transform shape] {\includegraphics[width=1cm]{obstacle}};
		}
	}
}
]
